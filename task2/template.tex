...
It provides two point lights at a distance of $2\rm e -5 \rm m $in \terxtit{aperture_2_spot.m}.
...
In the same way, we use a disk aperture as filter. 
 ...
 \begin{figure}[ht]
    \centering
    \includegraphics[width=0.35\textwidth]{task5-6_pic/3a.bmp}
    ...
    \label{resolution}
\end{figure}
\paragraph{To simplify the calculation, we consider the case of one spot incidece. 

As shown in Figure \ref{ocp}, we can see the light as a point source behind plane P1.
SO it is plane wave before plane P2. For coherent light, 
by choosing a circle aperture with radius a, the field of P3 in polar coordinates is given by,
\begin{aligned}
    U(r_i)&=\frac A {j\lambda f}
    \int _{-\infty} ^{+\infty}\int_0 ^{2\pi}
    \circ{(\frac r a )\exp{(j\frac {k} f r_i r \cos \theta)}}r\rm d r \rm d \theta \\
    &=\frac A{j\lambda f}\frac{J_1(\frac{kar_i}{f}){\frac{kar_i}{f}}}
    \label{5-1}
\end{aligned}
According to Rayleigh Criterion, resolution is
\begin{equation}
    r=1.22\frac{\lambda f}a
    \label{rc}
\end{equation}
It is equal to $28\rm {\mu m}$ when aperture radius is $2\rm {cm}$.}
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8/textwidth]{task5-6_pic/5_system.png}
    \caption{The optical convolution processor}
    \label{ocp}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8/textwidth]{task5-6_pic/if1.bmp}
    \caption{sin source with frequency 1cm(left); field of the image with incoherent light incident(right)}
    \label{ocp}
\end{figure}
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8/textwidth]{task5-6_pic/if2.bmp}
    \caption{sin source with frequency 2cm(left); field of the image with incoherent light incident(right)}
    \label{ocp}
\end{figure}
\begin{figure}[t]
    \centering
    \includegraphics[width=0.8/textwidth]{task5-6_pic/if2.2.bmp}
    \caption{sin source with frequency 2.2cm(left); field of the image with incoherent light incident(right)}
    \label{ocp}
\end{figure}
\paragraph{\bf{Expalanation & Calculation:}\\
    The image filed in the optical convolution processor is given by,
    \begin{equation}
        U_i(x_i,y_i)=h(x_i,y_i)*g(x_i,y_i)
    \end{equation}
    where g is the transmittance function in P1, h is Frouier transform
     if P2. From the calculation of PSF, it goves,
     \begin{equation}
         U_i(x_i,y_i)=h'(x_i,y_i)*U_0(\frac {x_i}M,\frac{y_i}M) (\rm{(Coherent)})
     \end{equation}
     \begin{equation}
        |U_i(x_i,y_i)|^2=\kappa |h'(x_i,y_i)|^2*|U_0(\frac {x_i}M,\frac{y_i}M)|^2 (\rm{(incoherent)})
    \end{equation}
    From these equations, we find that h is equivalent to h'. So the coherent transfer function is 
    \begin{equation}
        H(x,y)=rect(\frac x {l})
    \end{equation}
    We can see the filter as a optical system. So coherent transfer function is $H(f_x,f_y)=\mathcal F \[h(x_i,y_i)\]$.
    So $h=sinc(lf_x)
    \begin{equation}
        g(x_i,y_i)=\sin{(d_x x_i)} \\
        G(f_x,f_y)=\mathcal F (g(x_i,y_i))=\delta (f_X \PM D_X)\\
        H(f_x,f_y)=rect(\frac {x_i} l)
    \end{equation}
    The frouier transform of image is 
    \begin{equation}
        U_i^F(f_x,f_y)=\delta(f_x-d_x)rect(\lambda f \frac {f_x} l)
    \end{equation}
